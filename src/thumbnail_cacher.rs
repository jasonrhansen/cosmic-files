...
impl ThumbnailCacher {
    ...
    pub fn get_cached_thumbnail(&self) -> CachedThumbnail {
        ...
        // TODO: Check for failed thumbnail.

        CachedThumbnail::RequiresUpdate(self.thumbnail_size.pixel_size())
    }
    ...
    pub fn update(&self, temp_file: NamedTempFile) -> Result<(), Box<dyn Error>> {
    ...
    fn is_cached_thumbnail_valid(&self) -> bool {
        let thumbnail_file = match File::open(&self.thumbnail_path) {
            Ok(file) => file,
            Err(_) => return false,
        };
        let decoder = png::Decoder::new(thumbnail_file);
        let reader = match decoder.read_info() {
            Ok(reader) => reader,
            Err(err) => {
                log::warn!("failed to decode {:?} as PNG: {}", self.thumbnail_path, err);
                return false;
            }
        };
